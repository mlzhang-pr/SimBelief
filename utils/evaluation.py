...
def plot_latents(latent_means,    # semi-circle
                 latent_logvars,
                 rewards_preds,
                 num_episodes,
    ...
    # plt.plot(range(latent_means.shape[0]), latent_means.mean(axis=0), 'k.-')
...
def vis_rew_pred(args, rew_pred_arr, goal, **kwargs):
    ...
    for state in env.states:
        cell = Rectangle((state[0], state[1]), width=1, height=1,
                         fc=cmap(rew_pred_arr[ptu.get_numpy(env.task_to_id(ptu.FloatTensor(state).to('cpu')))[0]]))
        ax.add_patch(cell)
        ax.text(state[0] + 0.5, state[1] + 0.5,
                rew_pred_arr[ptu.get_numpy(env.task_to_id(ptu.FloatTensor(state).to('cpu')))[0]],
...
def plot_discretized_belief_halfcircle(belief_rewards, center_points, env, observations):   # circle-belief
...
def visualize_bahavior(observations, env):
    ...
    for episode in range(num_episodes):
        for t_i, timestep in enumerate(timesteps):
            ...
            env.plot_behavior(torch.cat((  torch.from_numpy(observations[:1, :]),
                                        torch.from_numpy(observations[episode * episode_len + 1:
...
def visualize_latent_space(latent_dim, n_samples, decoder):   # semi-circle
...
def visualize_latent_space_tsne(latents, pred_rewards):
    # latents = ptu.FloatTensor(sample_random_normal(latent_dim, n_samples))  #  latent samples

    # pred_rewards = ptu.get_numpy(decoder(latents, None))
    # goal_locations = np.argmax(pred_rewards, axis=-1)

    goal_locations = pred_rewards
    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()

    latents=scaler.fit_transform(latents)

    # print(goal_locations.shape)
    latent_dim=3
    # embed to lower dim space - if dim > 2
    if latent_dim > 2:
        tsne = TSNE(n_components=2, verbose=1, perplexity=40, n_iter=300)
    ...
    # plt.legend(loc='center right', bbox_to_anchor=(1.25, 1), ncol=1)
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
    # fig.show()
    plt.savefig('./t-sne/latent3.png')
    return data, goal_locations
