...
class VariBadWrapper(gym.Wrapper):
    def __init__(self,
        ...
        try:
            self.horizon_bamdp = self.episodes_per_task * self.env._max_episode_steps # meta episodes in total
    ...
    def step(self, action):
        ...
        if self._normalize_actions:     # from [-1, 1] to [lb, ub]
            ...
            if 'PandaPush' in self.env_name:
                z_lb = -0.4
                z_ub = -0.35
                action[2] = z_lb + (action[2] + 1.) * 0.5 * (z_ub - z_lb)
                action[2] = np.clip(action[2], z_lb, z_ub)
                # action[2] = -0.4
        ...
        done_bamdp = False  # bamdp= n * mdp
        if self.done_mdp:  # bamdp中包含多个 mdp
...
# class VariBadWrapper_minigrid(gymnasium.Wrapper):
#     def __init__(self,
#                  env,
#                  episodes_per_task
#                  ):
