...
class Push(Task):
    def __init__(
        ...
        # self.object_position = np.array([0.0, 0.0, self.object_size / 2])
    ...
    def sample_tasks(self, n_tasks):
        ...
        for i in range(n_tasks):
            ...
            while distance(target, object_pos) < 0.15:
                target = self._sample_goal()
                object_pos = self._sample_object()
...
class PandaPushEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        task = Push(sim, n_tasks=n_tasks, reward_type=reward_type)
    ...
    def step(self, action):
        ...
        reward = float(self.task.compute_reward(self.task.get_achieved_goal(), self.task.get_goal()))
