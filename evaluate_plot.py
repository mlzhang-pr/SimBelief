...
class MetaLearner:
    ...
    def __init__(self, args):
        ...
        ''' 随机生成train eval任务 '''
        shuffled_tasks = np.random.permutation(unwrapped_env.get_all_task_idx())  #返回 task idex

        # 返回goal
        # unwrapped_env.get_all_task()
        
        self.goals=unwrapped_env.tasks
        # print(goals)
    ...
    #             # -----------------------------------------------------
    #             self.vae_storage.task_buffers[task].clear()
    ...
    def evaluate(self, tasks):
        ...
        # print('sample 1',task_samples.shape)
        ...
        # 加载模型
        encoder_model_path= './logs/AntSemiCircle-v0-local/sac__73__12_05_02_42_47/models/encoder1900.pt'
        rew_decoder_model_path='./logs/AntSemiCircle-v0-local/sac__73__12_05_02_42_47/models/reward_decoder1900.pt'
        agent_model_path='./logs/AntSemiCircle-v0-local/sac__73__12_05_02_42_47/models/agent1900.pt'
        state_decoder_model=''

        self.vae_test=VAE(self.args)
        ...
        self.agent_test.load_state_dict(torch.load(agent_model_path))
        self.vae_test.encoder.load_state_dict(torch.load(encoder_model_path))
        self.vae_test.reward_decoder.load_state_dict(torch.load(rew_decoder_model_path))
        ...
        for task_idx, task in enumerate(tasks):
            ...
            with torch.no_grad():
                task_sample, task_mean, task_logvar, hidden_state = self.vae_test.encoder.prior(batch_size=1)
                # print("sample from encoder prior",task_sample.shape)  # [1,1,5]
            ...
            # print('task_sample[0, 0]',task_sample[0, 0].shape)   # shape[5]
            task_means[task_idx, step, :] = ptu.get_numpy(task_mean[0, 0])
            task_logvars[task_idx, step, :] = ptu.get_numpy(task_logvar[0, 0])

            if self.args.policy == 'ddqn':
                reward_preds[task_idx, step] = ptu.get_numpy(self.vae_test.reward_decoder(task_sample, None)[0, 0])
            else:
            ...
            for episode_idx in range(num_episodes):
                ...
                for step_idx in range(num_steps_per_episode):
                    ...
                    if self.args.policy == 'ddqn':
                        action, value = self.agent_test.act(obs=augmented_obs, deterministic=False)
                    else:
                        action, _, _, log_prob = self.agent_test.act(obs=augmented_obs,
                    ...
                    # print("from encoder",task_sample.shape) # [1,1,5]
                    if self.args.fixed_latent_params:
                    ...
                    if self.args.policy == 'ddqn':
                        reward_preds[task_idx, step + 1, :] = ptu.get_numpy(
                            self.vae_test.reward_decoder(task_sample, None)[0])
                        values[task_idx, step] = value.item()
                    else:
                        rewards[task_idx, step] = reward.item()
                        reward_preds[task_idx, step] = ptu.get_numpy(self.vae_test.reward_decoder(task_sample, next_obs, obs, action)[0, 0])
        ...
        # self.env.reset(44)
        # utl_eval.visualize_bahavior(observations[44], self.env)
        # utl_eval.plot_visited_states(observations[44], self.env)
        if self.args.policy == 'ddqn':
    ...
    def log(self, iteration, train_stats):
        ...
        if self.args.log_tensorboard:
            ...
            if self.args.policy == 'ddqn':
                ...
                for i, task in enumerate(self.train_tasks[:5]):
                    ...
                    #                                  utl_eval.plot_discretized_belief_halfcircle(reward_preds_train[i, :], [[1,0], [3, 0], [5, 0]], self.env, observations[i]),
                    #                                  self._n_env_steps_total)
                    
                    self.tb_logger.writer.add_figure('visited_states/task_{}'.format(i),
                                                     utl_eval.plot_visited_states(observations[i], self.env),
                                                     self._n_env_steps_total)
                    self.tb_logger.writer.add_figure('visualize_bahavior/task_{}'.format(i),
                                                     utl_eval.visualize_bahavior(observations[i], self.env),
                                                     self._n_env_steps_total)
            ...
            if self.args.max_rollouts_per_task > 1:
                ...
                self.tb_logger.writer.add_scalar('returns_multi_episode/sum',
                                                 np.mean(np.mean(returns_train, axis=-1)), #######
                                                 self._n_env_steps_total)
                ...
                if self.args.policy != 'ddqn':
                    self.tb_logger.writer.add_scalar('returns_multi_episode/sum_eval',
                                                     np.mean(np.mean(returns_eval, axis=-1)),  #######
                                                     self._n_env_steps_total)
        ...
        # print("Iteration -- {}, Success rate train -- {:.3f},  "
        #       "Avg. return train -- {:.3f}, Elapsed time {:5d}[s]"
        #       .format(iteration, np.mean(success_rate_train),
        #                np.mean(np.sum(returns_train, axis=-1)),
        ...
        for rollout in range(num_rollouts):
            ...
            while not done_rollout:
                ...
                if self.args.belief_reward:
                    if self.args.policy == 'ddqn' and self.args.oracle_belief_rewards:
