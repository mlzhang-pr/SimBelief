...
def get_args(rest_args):
    ...
    parser.add_argument('--vae-buffer-size', type=int, default=1e5, help='buffer size for VAE updates')  # 1e5
    # env
    parser.add_argument('--env-name', default='HalfCheetahVelSparse-v0', help='environment to train on')
    parser.add_argument('--max-rollouts-per-task', type=int, default=2)
    parser.add_argument('--num-tasks', type=int, default=120, help='number of goals in environment')
    parser.add_argument('--num-train-tasks', type=int, default=100, help='number of tasks for train')
    ...
    parser.add_argument('--num-tasks-sample', type=int, default=5, help='number of tasks to collect rollouts per iter')  ### 5

    parser.add_argument('--num-iters', type=int, default=1000, help='number meta-training iterates')
    parser.add_argument('--rl-updates-per-iter', type=int, default=2000, help='number of RL steps per iteration')  # 2000  1500?
    parser.add_argument('--vae-updates-per-iter', type=int, default=20, help='number of VAE steps per iteration')  # 20 25?  
    parser.add_argument('--num-rollouts-per-iter', type=int, default=5, help='number of rollouts to collect per task')
    parser.add_argument('--num-init-rollouts-pool', type=int, default=5,    ################### defalt = 5,   2 for debug  
    ...
    parser.add_argument('--task-embedding-size', type=int, default=10, help='dimensionality of latent space') # default=5
    ...
    parser.add_argument('--reward-decoder-layers', nargs='+', type=int, default=[64, 64])
    ...
    parser.add_argument('--gpu-id', type=int, default=4, help='gpu number to use')
