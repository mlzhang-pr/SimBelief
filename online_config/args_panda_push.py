...
def get_args(rest_args):
    ...
    parser.add_argument('--vae-buffer-size', type=int, default=5e4, help='buffer size for VAE updates')
    # env
    parser.add_argument('--env-name', default='PandaPush-v0', help='environment to train on')
    parser.add_argument('--obj_goal_range', default=0.2, help='range of init obj pos')

    parser.add_argument('--max-rollouts-per-task', type=int, default=3)   ################3
    parser.add_argument('--num-tasks', type=int, default=60, help='number of goals in environment')
    parser.add_argument('--num-train-tasks', type=int, default=50, help='number of tasks for train')
    parser.add_argument('--num-eval-tasks', type=int, default=10, help='number of tasks for evaluation')
    ...
    parser.add_argument('--num-tasks-sample', type=int, default=5, help='number of tasks to collect rollouts per iter')  # 10  # defalt=5

    parser.add_argument('--num-iters', type=int, default=2000, help='number meta-training iterates')     # 4000         # defalt=2000
    parser.add_argument('--rl-updates-per-iter', type=int, default=1000, help='number of RL steps per iteration')  # defal=1000
    parser.add_argument('--vae-updates-per-iter', type=int, default=25, help='number of VAE steps per iteration')
    ...
    parser.add_argument('--vae-batch-num-rollouts-per-task', type=int, default=3,    #######
    ...
    parser.add_argument('--actor-lr', type=float, default=0.00007, help='learning rate for actor (default: 7e-5)')
    parser.add_argument('--critic-lr', type=float, default=0.00007, help='learning rate for critic (default: 7e-5)')
    ...
    parser.add_argument('--vae-lr', type=float, default=0.001, help='learning rate for VAE (default: 3e-4)')
    parser.add_argument('--kl-weight', type=float, default=0.1, help='weight for the KL term')
    ...
    parser.add_argument('--decode-state', type=boolean_argument, default=True, help='use state decoder')
    parser.add_argument('--state-loss-coeff', type=float, default=1.0, help='weight for state loss (vs reward loss)')
    parser.add_argument('--state-decoder-layers', nargs='+', type=int, default=[64, 64])
    parser.add_argument('--state-pred-type', type=str, default='gaussian',  # if use bisimulation switch to gaussian
                        help='choose from: bernoulli, gaussian, deterministic')
